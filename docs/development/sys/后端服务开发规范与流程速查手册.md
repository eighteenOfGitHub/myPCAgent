...
*   **示例 (LLM Setting Endpoints - 使用拆分后的模型)**:
    ...
    # from backend.db_models.user_config import LLMConfig  # 移除：不直接作为 response_model
    ...
    def list_llm_configs():
        ...
        service = LLMSettingService()
        try:
            configs = service.get_all()  # 返回 DB 模型列表
            return configs  # FastAPI 基于 from_attributes 转为 LLMConfigResponse
        except Exception as e:
...
8.  **响应体安全**: 
    ...
    *   明确定义返回字段,避免意外暴露数据
9.  **业务筛选位置建议**: 业务筛选、字段裁剪、权限/可见性等规则应放在 service 层，便于复用与集中管理，API 层保持薄、负责路由与输入校验即可；若放在 API 层会导致多处重复和业务分散，不利于维护。
